...
export default function AddTaskScreen() {
  ...
  const handleAddTask = async () => {
    ...
    try {
      ...
      if (data) {
        //* data varsa taskların ID'si ile data id birbirine eşit ise yeni objeyi gönder değilse task kalsın.
        tasks = tasks.map(task => (task.id === data.id ? newTask : task));
      } else {
        //* data yoksa yeni oluşturulan veriyi tasks dizisine ekle.
        tasks.push(newTask);
      }
      //* Veriyi AsyncStorage'ye string olarak ekledik.
      await AsyncStorage.setItem('tasks', JSON.stringify(tasks));

      Toast.show({
        type: 'success',
        text1: data ? 'Task has Been Uptated' : 'Task has Been Added',
        topOffset: 60,
      });

      //* TaskList screen'e navigate ile yönlendirdik.
